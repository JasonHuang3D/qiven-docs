...
## 4. Installed-Desktop loading (Phase B — dev-mode full chain proven; packaged-isolation residual open)
...
- **Full Desktop chain proven via the repository's dev entry** (owner-approved continuation): `pnpm dev:desktop:test` under the same scratch isolation rebuilt the patched agent (fresh `dist/zcode.cjs`) and started an Electron dev app ("ZCode Dev", separate identity). The dev Desktop spawned the patched agent — `electron.exe` as Node running `apps/zcode-cli/packages/cli/dist/zcode.cjs app-server --stdio --surface desktop` — and one owner-sent GUI message produced the complete evidence chain in order: `logical` (operation=agent_step, actorKind=main, msgCount=6, toolCount=33 on the desktop surface), `final-permit` (attempt=1, msgCount=4, projectionDigest), then the mock provider's wire receipt (streamed, msgCount=4, matching the final projection). The two-phase projection change reproduces identically on the Desktop path.
- Residual: the isolated PACKAGED instance still exits cleanly (exit 0, no crash entry) after early init + renderer/GPU init, before any workspace session can spawn an agent — undiagnosed, lower priority now that the dev route delivers the full chain. The eventual qualified trial (H-4) targets the packaged installed profile and will need this resolved or bypassed by the packaging chain (stage + `pnpm bundle:desktop`).
