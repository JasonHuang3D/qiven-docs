...
1. **`BeforeModelInvocation`:** every logical model call in the qualified Desktop Agent, including later turns, auxiliary calls and subagents, obtains a fresh Qiven classification before projection. Qiven may activate, revalidate reuse of a task bundle, admit a named observed-only class, or deny. Reuse does not skip classification. A design-capable class must place its exact bounded renderer output in model-visible input before the first design-producing call; an observed-only class must be proven unable to affect governed design or actions.
