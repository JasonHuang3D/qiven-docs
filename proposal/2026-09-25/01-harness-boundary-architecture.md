...
The minimal physical contract has **two model boundaries**, plus the action boundary:

1. **Logical invocation preflight:** before a governed call is projected into provider format, acquire or reuse a valid task bundle and place its exact bounded renderer output in model-visible input. This boundary is useful for task/phase resolution and fast rejection but is not final proof of delivery.
2. **Physical attempt authorization:** after any retry, model-specific projection, header refresh and options construction, and immediately before `AiSdkModelRuntime.generateText` or `.streamText`, compare final provider-facing content against the registered injection. Observe dispatch and stream/result correlation. Every actual attempt passes this guard.
3. **Action mediation:** before an in-scope tool dispatch, observe structured arguments and resource targets, obtain a Qiven decision and preserve ZCode/Host permission handling. Observe terminal and indeterminate outcomes afterward.

The scope is the specified `DeploymentProfile`, not a blanket claim about every use of ZCode. The first profile must enumerate process/entrypoint, parent and subagent roles, model providers and invocation classes, tool namespaces, background tasks, and alternate binary paths. Unknown or unobserved call sites are a coverage failure for the affected class. A `UserPromptSubmit` event may help form the task descriptor, but an internal retry, continuation, compaction, reviewer call or delegated subagent may start without that event.
...
| --- | --- | --- |
| `InvocationRef` | process/build ID, session ID, role scope, task ID, phase epoch, logical call ID, model identity, stream flag | Uniqueness across concurrent calls and resumed sessions; no bare session-wide mutable `currentReceipt` |
...
| `PlacementVerifiedEvent` | invocation/attempt, receipt and bundle digests, renderer build, final message projection digest, insertion location, monotonic ordering marker | Emitted before send only after exact projected-content match; does **not** itself prove delivery |
...
Task establishment comes from an **observed external boundary**: a captured prompt submission, a delegated subagent brief, a designated workflow/goal entry, or another inventoried ingress. It may use claimed task fields only as labelled evidence. Runtime normalizes paths/revisions against verified local roots, exact source locks, phase and risk enums; unknown critical applicability is visible. A model's phrasing alone cannot prove an observed boundary or H2 authority.

For each task, maintain a phase epoch and one immutable activation keyed by `(task digest, execution generation, activation generation, renderer, consumer profile, role scope, phase epoch)`. Concurrent subagents receive distinct invocation references even if sharing source material. Within an epoch, retries may reuse the immutable selection after freshness validation; a changed target, risk, boundary kind, source revision or phase creates a new epoch and requires reactivation before further governed design output. Do not retroactively use a new receipt to qualify earlier output. A design digest is bound later in Devkit design evidence, never forged into a pre-design receipt.

On the first design-producing invocation, the adapter inserts a **single canonical Qiven segment** in a harness-approved location that is guaranteed to be model-visible for that provider and role. It identifies the task/receipt, the selected binding and hazards, and a short recovery instruction for unresolved protected applicability. The provider-specific insertion mapping, instruction priority, truncation behavior and maximum supported size are registered and tested; if system or developer messages are disallowed, the adapter selects a qualified alternative placement or blocks the governed call. Prevent duplicate additions on retry or restoration; compare the stable segment ID and content digest. Ordinary later turns need a consistent, budgeted refresh policy, rather than blind full reinjection on every token-producing call. `ReadyForPhase` does not mean a new general-purpose permission to execute.

Role-aware delivery obeys ADR-0053: full canonical cold boot for main `jason-brother`/`jason-extended-cognition`; a `jason-worker` subagent receives the delegator's sealed scoped brief and applicable engineering invariants without a full cold boot. A role tag cannot remove a protected rule that applies to the worker's action. The owner-only governance/red-line constraint is enforced at the action boundary irrespective of which model or role generated the proposal.
...
| Final attempt content differs, disappears or arrives late | No dispatch, no backdated event | Rebuild request at a new invocation/attempt |
...
- If the adapter is disabled or an older `zcode.cjs` is loaded, startup and trial preflight must show an explicit `UnqualifiedAdapter` state for governed work. Never treat a prior session's success as evidence for a newly loaded binary.
