...
Reconcile this guard with the native attempt seam first: both runners already await `admitAttempt` (the factory-bound `modelRequestAdmission` port) immediately before every send, with cancellation classified as a connect-phase failure and the ticket receiving the attempt's status events. The preferred patch extends that existing wait point with the Qiven decision and projection proof (or adds the check directly beside it inside the same critical section), rather than installing a second, unrelated pre-send wait that must independently re-derive queuing, cancellation and status semantics.
